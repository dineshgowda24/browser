...
func TestNewWii(t *testing.T) {
	Convey("Given a user agent string", t, func() {
		So(NewWii(testDevices["wii-1"]), ShouldHaveSameTypeAs, &Wii{})
	})
}
	...
	Convey("Subject: #Match", t, func() {
		Convey("When the user agent matches", func() {
			Convey("It should return true", func() {
				So(NewWii(testDevices["wii-1"]).Match(), ShouldBeTrue)
				So(NewWii(testDevices["wii-2"]).Match(), ShouldBeTrue)
				So(NewWii(testDevices["wii-3"]).Match(), ShouldBeTrue)
			})
		})

		Convey("When the user agent does not match", func() {
			Convey("It should return false", func() {
				So(NewWii(testDevices["bb-playbook-1"]).Match(), ShouldBeFalse)
			})
		})
	})
}
