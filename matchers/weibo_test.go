...
func TestNewWeibo(t *testing.T) {
	Convey("Given a user agent string", t, func() {
		So(NewWeibo(""), ShouldHaveSameTypeAs, &Weibo{})
	})
}
...
func TestWeiboVersion(t *testing.T) {
	Convey("Subject: #Version", t, func() {
		Convey("It should return default version", func() {
			...
			So(NewWeibo(ua.Android).Version(), ShouldEqual, "6.5.0")
			So(NewWeibo(ua.IOS).Version(), ShouldEqual, "10.9.2")
		})
	})
}
	...
	Convey("Subject: #Match", t, func() {
		Convey("When the user agent matches", func() {
			Convey("It should return true", func() {
				ua := testUserAgents["weibo"]
				So(NewWeibo(ua.Android).Match(), ShouldBeTrue)
				So(NewWeibo(ua.IOS).Match(), ShouldBeTrue)
			})
		})
		...
		Convey("When the user agent doesn't match", func() {
			Convey("It should return false", func() {
				...
				So(NewWeibo(ua.Android).Match(), ShouldBeFalse)
			})
		})
	})
}
