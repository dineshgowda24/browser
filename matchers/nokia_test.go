...
func TestNewNokia(t *testing.T) {
	Convey("Subject: #NewNokia", t, func() {
		Convey("It should return a new Nokia instance", func() {
			So(NewNokia(""), ShouldHaveSameTypeAs, &Nokia{})
		})
	})
}
...
func TestNokiaVersion(t *testing.T) {
	Convey("Subject: #Version", t, func() {
		...
		Convey("When the version is matched", func() {
			Convey("It should return the version", func() {
				So(NewNokia(ua.Android).Version(), ShouldEqual, "1.2.0.12")
				So(NewNokia(ua.Linux).Version(), ShouldEqual, "2.3.0.0.48")
			})
		})
	})
	...
	Convey("When the version is not matched", t, func() {
		Convey("It should return default version", func() {
			n := NewNokia(testUserAgents["chrome"].Linux)
	...
	Convey("Subject: #Match", t, func() {
		Convey("When user agent matches Nokia", func() {
			Convey("It should return true", func() {
				...
				So(NewNokia(ua.Android).Match(), ShouldBeTrue)
				So(NewNokia(ua.Linux).Match(), ShouldBeTrue)
			})
		})
		...
		Convey("When user agent does not match Nokia", func() {
			Convey("It should return false", func() {
				...
				So(NewNokia(ua.Linux).Match(), ShouldBeFalse)
			})
		})
	})
}
